...
// Code uses ENGORD^PSGOU. My package; no ICR necessary.
module.exports.handlers.getInpatientMedications = function(messageObj, session, send, finished) {
  ...
  this.db.use('TMP', 'PSJ', process.pid).forEachChild((orderType) => {
    ...
    switch (true) {
      case orderType.$p(1) === 'Cz': clinicData = orderType; break; 
    ...
    if (clinicData) {
      result[orderType].clinicName = clinicData.$p(2);
      result[orderType].clinicSortPosition = clinicData.$p(3);
      let clinicOrderType = clinicData.$p(4);
      switch (true) {
        case clinicOrderType === 'A': // fallthrough intended
        case clinicOrderType === 'DF': orderTypeText = this.db.function({function: 'TXT^PSJO', arguments: [ clinicOrderType ] }).result; break;
        case clinicOrderType.indexOf('CC') > -1: orderTypeText = this.db.function({function: 'TXT^PSJO', arguments: [ 'PR' ] }).result; break;
        case clinicOrderType.indexOf('CD') > -1: orderTypeText = this.db.function({function: 'TXT^PSJO', arguments: [ 'PC' ] }).result; break;
        case clinicOrderType.indexOf('C') > -1: orderTypeText = this.db.function({function: 'TXT^PSJO', arguments: [ 'P' ] }).result; break;
        case clinicOrderType.indexOf('BD') > -1: orderTypeText = this.db.function({function: 'TXT^PSJO', arguments: [ 'NC' ] }).result; break;
        case clinicOrderType.indexOf('B') > -1: orderTypeText = this.db.function({function: 'TXT^PSJO', arguments: [ 'N' ] }).result; break;
        case clinicOrderType === 'O': orderTypeText = this.db.function({function: 'TXT^PSJO', arguments: [ 'NA' ] }).result; break;
      }
    }
    result[orderType].orderTypeText = orderTypeText;
    result[orderType].medicationData = {};
    this.db.use('TMP', 'PSJ', process.pid, orderType).forEachChild((medType, node) =>{
      ...
      result[orderType].medicationData[medTypeText] = new Array();
      node.forEachChild((med_n_on) => result[orderType].medicationData[medTypeText].push(med_n_on.$p(1)));
    });
  });
  finished(result);
};
