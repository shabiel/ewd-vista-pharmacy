...
module.exports.handlers.getPatientDemographics = function(messageObj, session, send, finished) {
  //TODO: THIS IS HACKEY!!! Call RPC instead.
  this.db.symbolTable.restore(session);
  this.db.symbolTable.setVar('FILTER("patientId")', messageObj.params.DFN);
  this.db.symbolTable.setVar('FILTER("domain")', 'PATIENT');
  this.db.procedure({procedure: 'GET+1^HMPDJ'});
  let fullJSON = '';
  this.db.use('TMP', 'HMP', process.pid).forEachLeafNode( (value) => fullJSON += value);
  finished(JSON.parse(fullJSON));
};
