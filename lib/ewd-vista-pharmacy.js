...
module.exports.handlers.getPatientAllergies = function(messageObj, session, send, finished) {
  ...
  if (mainStatusOutput === '') {
    ...
    finished({
      ...
      statusCode: 'NAA' // That's Sam's invention--doesn't mean anything.
    });
    return;
  }
...
// IA 4682 -> NKA^GMRAGUI1 to mark patient as NKA.
module.exports.handlers.markPatientAllergies = function(messageObj, session, send, finished) {
  if (messageObj.params.isNKA) {
    this.db.symbolTable.setVar('ORDFN', messageObj.params.DFN);
    this.db.procedure({ procedure: 'NKA^GMRAGUI1' });
    finished({});
    return;
  }
};

// IA # 5873 (ORWDAL32 DEF) to get Top Ten signs and symptoms - Not active
// IA # 6457 (ORWDAL32 SYMPTOMS) to get the initial list of Signs and Symptoms
// - Not public.
module.exports.handlers.getAllergyDialogData = function(messageObj, session, send, finished) {
  let result = {};
  this.db.symbolTable.restore(session);
  this.db.symbolTable.killVar('ZZZ');
  this.db.function({function: 'D^ewdVistAUtils', arguments: ['DEF^ORWDAL32(.ZZZ)']});
  ...
  if (this.db.symbolTable.getVar('$O(ZZZ(0))')) {
    ...
    // ZZZ(11)="iF^Food"
    // ZZZ(12)="iO^Other"
    // ZZZ(13)="iDF^Drug,Food"
    // ZZZ(14)="iDO^Drug,Other"
    // ZZZ(15)="iFO^Food,Other"
    // ZZZ(16)="iDFO^Drug,Food,Other"
    // ZZZ(17)="~Nature of Reaction"
    // ZZZ(18)="iA^Allergy"
    //
    let topTenSS = [];
    for (let i = 1; i !== ''; i = this.db.symbolTable.getVar(`$O(ZZZ(${i}))`)) {
      let v = this.db.symbolTable.getVar(`ZZZ(${i})`);
      if (!start && v !== '~Top Ten') continue;
      if (v === '~Top Ten') {
        start = true;
        continue;
      }
      if (v[0] === '~') break; // We are done.

      let ssText = v.slice(1); // Remove the 'i' at the beginning
      let ss = {};
      ss.ien = ssText.$p(1);
      ss.name = ssText.$p(2);
      topTenSS.push(ss);
    }
    result.topTenSS = topTenSS;
  }

  let allSS = [];
  this.db.symbolTable.killVar('ZZZ');
  this.db.function({function: 'D^ewdVistAUtils', arguments: ['SYMPTOMS^ORWDAL32(.ZZZ,"",1)']});
  // ZZZ(1)="476^A FIB-FLUTTER"_$C(9)_"<ATRIAL FIBRILLATION-FLUTTER>^ATRIAL FIBRILLATION-FLUTTER"
  // ZZZ(2)="237^ABDOMINAL BLOATING"
  // ZZZ(3)="236^ABDOMINAL CRAMPS"
  // ZZZ(4)="429^ABDOMINAL DISCOMFORT"
  //
  if (this.db.symbolTable.getVar('$O(ZZZ(0))')) {
    for (let i = 1; i !== ''; i = this.db.symbolTable.getVar(`$O(ZZZ(${i}))`)) {
      let v = this.db.symbolTable.getVar(`ZZZ(${i})`);
...
// IA # 6457 (ORWDAL32 SYMPTOMS) to get the initial list of Signs and Symptoms
// - Not public.
module.exports.handlers.getAllergySignsSymptomsContinued = function(messageObj, session, send, finished) {
  let result = [];
  this.db.symbolTable.restore(session);
  this.db.symbolTable.killVar('ZZZ');
  this.db.function({function: 'D^ewdVistAUtils', arguments: [`SYMPTOMS^ORWDAL32(.ZZZ,"${messageObj.params.from}",1)`]});
  if (this.db.symbolTable.getVar('$O(ZZZ(0))')) {
    for (let i = 1; i !== ''; i = this.db.symbolTable.getVar(`$O(ZZZ(${i}))`)) {
      let v = this.db.symbolTable.getVar(`ZZZ(${i})`);
      let ss = {};
...
// IA # 6455 (ORWDAL32 ALLERGY MATCH) to search for allergens -- Not public
module.exports.handlers.searchforAllergens = function(messageObj, session, send, finished) {
  let result = {};
  this.db.symbolTable.restore(session);
  this.db.symbolTable.killVar('ZZZ');
  this.db.function({function: 'D^ewdVistAUtils', arguments: [`ALLSRCH^ORWDAL32(.ZZZ,"${messageObj.params.from}")`]});
  // VADEMO201608>K XXX D ALLSRCH^ORWDAL32(.XXX,"SULFON")
  //
  // VADEMO201608>ZWRITE XXX
  // XXX(1)="1^VA Allergies File^^^TOP^+"
  // XXX(2)="136^SULFONAMIDES/RELATED^GMRD(120.82,""B"")^D^1"
  // XXX(3)="144^SULFA DRUGS <SULFONAMIDE RELATED DRUGS>^GMRD(120.82,""D"")^D^1"
  // XXX(4)="144^SULFA DRUGS <SULFONAMIDES>^GMRD(120.82,""D"")^D^1"
  // XXX(5)="144^SULFA DRUGS <SULFONYLUREAS>^GMRD(120.82,""D"")^D^1"
  // XXX(6)="3^National Drug File - Generic Drug Name^^^TOP^+"
  // XXX(7)="2032^SULFONATED CASTOR OIL^PSNDF(50.6,""B"")^D^3"
  // XXX(8)="5008^SULFONATED PHENOL/SULFURIC ACID^PSNDF(50.6,""B"")^D^3"
  // XXX(9)="4^National Drug file - Trade Name^^^TOP^+"
  // XXX(10)="5^Local Drug File^^^TOP^+"
  // XXX(11)="7^Drug Ingredients File^^^TOP^+"
  // XXX(12)="2036^SULFONATED CASTOR OIL^PS(50.416,""P"")^D^7"
  // XXX(13)="3094^SULFONATED ALKYL BENZENES^PS(50.416,""P"")^D^7"
  // XXX(14)="4986^SULFONATED PHENOL^PS(50.416,""P"")^D^7"
  // XXX(15)="8^VA Drug Class File^^^TOP^+"
  // XXX(16)="23^SULFONAMIDE/RELATED ANTIMICROBIALS^PS(50.605,""C"")^D^8"
  //
  if (this.db.symbolTable.getVar('$O(ZZZ(0))')) {
    result.categories = {};
    result.allergens = [];
    for (let i = 1; i !== ''; i = this.db.symbolTable.getVar(`$O(ZZZ(${i}))`)) {
      let v = this.db.symbolTable.getVar(`ZZZ(${i})`);
      if (v.$p(6) === '+') {
        result.categories[v.$p(1)] = v.$p(2);
      }
      else {
        let allergen = {};
        allergen.ien = v.$p(1);
        allergen.name = v.$p(2);
        allergen.global = v.$p(3);
        allergen.type = v.$p(4);
        allergen.category = v.$p(5);
        result.allergens.push(allergen);
      }
    }
  }
...
// IA#4682: Use of UPDATE^GMRAGUI1
module.exports.handlers.addOrEditADRRecord = function(messageObj, session, send, finished) {
  ...
  let p = messageObj.params;

  if (p.adrIEN) this.db.symbolTable.setVar('GMRAIEN', p.adrIEN);
  else          this.db.symbolTable.setVar('GMRAIEN', 0);

  g.$('GMRAGNT').value = p.allergen.name + '^' + p.allergen.ien + ';' + p.allergen.global.$p(1,',') + ',';
  g.$('GMRATYPE').value = p.allergen.typeInternal;
  g.$('GMRANATR').value = p.reactionTypeInternal;
  g.$('GMRAORIG').value = DUZ;
  g.$('GMRAORDT').value = nowFM;
  g.$('GMRASYMP').$(0).value = p.ss.length;
  p.ss.forEach((one, index) => {
    g.$('GMRASYMP').$(index + 1).value = one.ien + '^' + one.name;
  });

  g.$('GMRACHT').$(0).value = 1;
  g.$('GMRACHT').$(1).value = nowFM;
  g.$('GMRAOBHX').value = p.observationType === 'historical' ? 'h' : 'o';
  g.$('GMRACMTS').$(0).value = p.comments.length ? 1 : 0;
  if (p.comments.length) g.$('GMRACMTS').$(1).value = p.comments;

  if (p.observationType == 'observed') {
    g.$('GMRARDT').value = p.fmObsDate;

    // Convert Mild, Moderate, Severe to a number
    this.db.symbolTable.killVar('ZZZ');
    this.db.function({function: 'D^ewdVistAUtils', arguments: [`CHK^DIE(120.85,14.5,"","${p.obsADRSeverity}",.ZZZ)`]});
    let obsSeverityInternal = this.db.symbolTable.getVar('ZZZ');
    assert(obsSeverityInternal !== '^');
    g.$('GMRASEVR').value = obsSeverityInternal;
  }

  this.db.symbolTable.killVar('ORY');

  this.db.function({function: 'D^ewdVistAUtils', arguments: 
    [`UPDATE^GMRAGUI1(GMRAIEN,${messageObj.params.DFN},$NA(^TMP("GMRA",$J)))`]});
